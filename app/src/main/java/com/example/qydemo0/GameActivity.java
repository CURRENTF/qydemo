...
public class GameActivity extends Activity {
                            ...
                            switch (mode) {
                                case 0:
                                    if(cur_star_num!=0) {
                                        ...
                                        if(cur_img_ind + 1 == img_list.size()){
    ...
    private Boolean success_pass(){
        try {
            String[] j = new String[0];
            JSONObject res_json = new JSONObject(cur_request.advancePut(GenerateJson.universeJson2(j), Constant.mInstance.game_url + "/free/"+gid_list.get(cur_img_ind)+"/", "Authorization", GlobalVariable.mInstance.token));
...
//
//            String[] callToJson = {"gid","int", ""+gid_list.get(cur_img_ind),
//                    "img","string",cur_img_id};
//            try {
//                JSONObject res_json = new JSONObject(cur_request.advancePost(GenerateJson.universeJson2(callToJson),Constant.mInstance.game_url+"/", "Authorization", GlobalVariable.mInstance.token));
//            } catch (JSONException e) {
//                e.printStackTrace();
//            }
//            if(cur_star_num > 0){
...
//            }
    ...
    public class getTZImageList extends AsyncTask<Integer, Void, List<String> >{
...
//            }
    ...
    public class getImageList extends AsyncTask<Void, Void, List<String> >{
